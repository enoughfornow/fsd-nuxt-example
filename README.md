...
Отображение модели model View - это компонент который "показывает", как выглядит данная сущность.
 ...
 например `entities/product/VProductCard`, `entities/auth/VAuthButton`, `entities/user/VUserAvatar` -
 представление - "карточка товара, кнопка авторизации, аватар юзера",
