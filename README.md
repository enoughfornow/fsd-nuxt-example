...
Отображение модели model View - это компонент который "показывает", как выглядит данная сущность. Все компоненты отображения модели должны иметь префикс `V`(view), например `VExampleCard` или `VUserProfile` - представление "карточки товара, юзера в профиле"
